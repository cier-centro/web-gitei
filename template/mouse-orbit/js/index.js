...
Canvas.prototype.setupParticles = function() {
  ...
  while(++index < this.options.count) {if (window.CP.shouldStopExecution(1)){break;}
    var x;
    var y;
    var angle;
...
Canvas.prototype.findClosest = function() {
  ...
  while(++index < pointsLength) {if (window.CP.shouldStopExecution(3)){break;}
    var closestIndex = -1;
    this.particles[index].closest = [];

    while(++closestIndex < pointsLength) {if (window.CP.shouldStopExecution(2)){break;}
...
Canvas.prototype.ghostGradient = function() {
  ...
  if(typeof this.options.background === 'string') {
    ...
    for(var i = 0; i < length; i++){if (window.CP.shouldStopExecution(4)){break;}
...
Canvas.prototype.draw = function() {
  ...
  while(++index < length) {if (window.CP.shouldStopExecution(5)){break;}
...
Canvas.prototype.drawLines = function (point, color) {
  ...
  while(++index < length) {if (window.CP.shouldStopExecution(6)){break;}
...
function Particle(options) {
  ...
  if(this.options.color) {
    var color = this.options.color.split(',');
	  var colorIndex = -1;
    while(++colorIndex < 3) {if (window.CP.shouldStopExecution(7)){break;}
...
new Canvas({
  ...
  color: '255, 123, 99',
  maxDistance: 110,
  background: ['152, 70, 142','0, 167, 183']
});
//# sourceURL=pen.js
